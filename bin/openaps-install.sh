...
curl -s https://raw.githubusercontent.com/buzlachok/oref0_original/$BRANCH/bin/openaps-packages.sh | bash -
mkdir -p ~/src; cd ~/src && ls -d oref0 && (cd oref0 && git checkout $BRANCH && git pull) || git clone https://github.com/buzlachok/oref0_original.git
